...
# 环境配置
- OS: Ubuntu 25.04
- 网络ip及端口：支持局域网和本机；控制端口2100，数据传输端口2000，被动模式5000以上
- 依赖：cmake
...
在bin目录下有ftpserver,ftpclient文件。

./ftpserver正常启动  

./ftpclient可加入参数，即服务端指定的ip地址和端口号，否则默认是连接到本机(127.0.0.1)

## 系列操作

1. 需要先在一个终端上启动服务端，之后可在其他终端上开启多个客户端。
2. 客户端在默认匹配时，当前的bin目录，即模拟了客户端的根目录。
3. 列出目录，上传，下载等系列操作都在客户端启动时有显示，用户可参照使用，或者执行help命令查询。
......
...
- 实现思路:
1. 采用TCP连接的系列系统调用，完成底层的连接框架。
2. 在此基础上完成信号注册，命令处理，文件传输等业务功能。

- 优化的方案:  
1. 支持局域网内连接，不同主机之间文件传输保持隔离。
2. 底层架构方面，使用epoll + ThreadPool的方式，优化了多客户端的连接。
3. 文件发送接收方面，使用sendfile()零拷贝技术，提高了文件传输的效率。
4. UI界面，采用哈希表，为每个命令绑定一个调用对象，为后续增加更多命令提供了方便的接口。
5. 客户端隔离方面，使用Session结构体封装。
6. 使用ThreadPool内先前写过的线程安全的日志处理方法，有较为清晰的日志显示。

## 有待后续优化方面

1. 美观的图形化界面。此处本身想使用Qt库来实现，后续可以加入。
2. 加入断点续传。
3. 优化用户的登录和注册。以及管理员身份的实现，为不同等级的用户赋予不同的权限，如管理员可以关闭服务器，其他人无法关闭。
4. 更高性能的服务器。后续可采用muduo网络库优化连接的处理，采用主从Reactor的模式或架构服务器。  
......